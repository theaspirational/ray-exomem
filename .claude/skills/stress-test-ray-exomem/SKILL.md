...
- **`belief-row` returns one row per `belief_id` in the selected branch view, not one row per revision.** Revising/revoking a belief appends a branch-local belief row, and the branch view collapses to the latest row per id. Don't expect a separate `superseded` row to land in the view — revision history is visible through the tx log (`tx/action = "revise-belief"` / `"revoke-belief"`).
- **Branches are request parameters, not daemon state.** There is no `current_branch`, no branch-switch endpoint, and no `is_current` field on branch rows. Every branch-sensitive read/write passes `branch` explicitly and defaults to `main`. `create_branch` uses `parent_branch_id` (default `main`); `merge_branch` must name a `target_branch`.
- **Observations are exom-level in the API/UI.** Facts, beliefs, relation graph/schema samples, query/eval, and timeline are branch-view surfaces. Observations and rules are exom-level; observations carry their origin branch via the creating transaction, but `/api/observations?exom=...` is not filtered by selected branch.
...
| 2 | `phase-2-branches-sessions.md` | create_branch(parent) / explicit-branch reads+writes / merge(target) / archive, session_new (single+multi) / close / join unknown, init + exom_new with `acl_mode`, folder_new (idempotent + reject-on-exom), rename (folder + exom + reject namespace-root + reject session-id), delete (folder + exom + recursive + reject namespace-root + reject missing), exom_mode MCP flip (creator-only + audit fact + no-op + session-rejection), regression probes (hyphen attr / default-fact-id supersede / sym health / cache staleness / no cursor state), branch-param API/UI smoke | MCP (user1) + Chrome when available |
