...
fn rename_folder_cascades() {
    ...
    let resp: serde_json::Value =
        ureq::post(&format!("{}/ray-exomem/api/actions/rename", d.base_url))
    ...
    let tree: serde_json::Value =
        ureq::get(&format!("{}/ray-exomem/api/tree?path=work", d.base_url))
            .call()
            .unwrap()
            .into_json()
            .unwrap();
...
fn rename_rejects_case_collision() {
    let d = TestDaemon::start();
    ureq::post(&format!("{}/ray-exomem/api/actions/init", d.base_url))
        .send_json(json!({"path":"work::foo"}))
        .unwrap();
    ureq::post(&format!("{}/ray-exomem/api/actions/init", d.base_url))
        .send_json(json!({"path":"work::bar"}))
        .unwrap();
    ...
    if let ureq::Error::Status(s, _) = err {
        ...
        panic!("expected case-collision error");
    }
}
